...
static void mt9v03x_vsync_handler_1(void)
{
    ...
    else
    {
        if(mt9v03x_gather_flag == 1)
        {
            ...
            dma_disable(MT9V03X_1_DMA_CH);
            // Copy a complete frame before DMA restarts, so the app never reads a half-written image.
            for(uint32 i = 0; i < MT9V03X_1_IMAGE_SIZE; i++)
            {
                (&mt9v03x_image_2[0][0])[i] = (&mt9v03x_image_1[0][0])[i];
            }
